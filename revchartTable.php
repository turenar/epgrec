...
if( EXTRA_TUNERS > 0 ){
	for( $cnt=0; $cnt<EXTRA_TUNERS; $cnt++ ){
...
for( $st=0; $st<$lp_lmt; $st++ ){
	...
	for( $t_lp=0; $t_lp<$t_lmt; $t_lp++ ){
		$reca = $prec->fetch_array( null, null, $tuners_que[$t_lp].' AND endtime>"'.toDatetime($ch_top_time).'" AND starttime<"'.toDatetime($ch_last_time).'" ORDER BY starttime ASC' );
		if( count($reca) ){
			$prev_end = $ch_top_time;
			$num_ch++;
			$program  = array();
			$program['name']          = $tname[$t_lp];
